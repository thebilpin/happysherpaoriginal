		...
		<div class="page-title d-flex">
			<h4>
				<span class="font-weight-bold mr-2">Reviews of <a class="ml-1"
						href="{{ route('admin.get.editRestaurant', $restaurant->id) }}">{{ $restaurant->name }}</a></span>
				<span
					class="ml-1 badge badge-flat text-white {{ ratingColorClass($averageRating) }}">{{ $averageRating }}
			...
			<div class="table-responsive">
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td><a
									href="{{ route('admin.viewOrder', $review->order->unique_order_id) }}">{{ $review->order->unique_order_id }}</a>
							</td>
							<td><a
									href="{{ route('admin.get.editUser', $review->user->id) }}">{{ $review->user->name }}</a>
							</td>
							<td>
							...
							<td>
								<div class="d-flex">
									<div class="mr-1">
										<span
											class="ml-1 badge badge-flat text-white {{ ratingColorClass($review->rating_delivery) }}"
											data-popup="tooltip" title="Delivery Review"
											data-placement="top">{{ $review->rating_delivery }} <i
								...
								<div class="d-flex">
									<div class="mr-1">
										<span
											...
											data-placement="top">{{ $review->rating_store }} <i
												class="icon-star-full2 text-white"
												style="font-size: 0.6rem;"></i></span>
									</div>
									<div>
							...
							<td class="text-center">
								...
								<button class="btn btn-sm btn-primary reviewViewButton" data-toggle="modal"
									data-target="#viewReviewModal" data-reviewId="{{ $review->id }}"
									data-ratingDelivery="{{ $review->rating_delivery }}"
									data-ratingStore="{{ $review->rating_store }}"
									data-reviewDelivery="{{ $review->review_delivery }}"
									data-reviewStore="{{ $review->review_store }}"> View </button>
